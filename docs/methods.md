...
Size follows [GenZui Serif v0.113](https://github.com/mkpoli/GenZui/releases/tag/v0.113), whose 〱 is 1,840 units tall with a 2,000-unit vertical advance. Early forms scaled into one em were rejected as too small beside it, and the vertical advance was set to two ems to match it, since GenZui's mark takes two cells in vertical text. [BabelStone Han](https://www.babelstone.co.uk/Fonts/Han.html), which sets the mark in one em, was shown beside the early rounds. The reading samples are lines that use 〱 and 〲 as transcribed on みんなで翻刻 ([honkoku-data v3](https://github.com/yuta1984/honkoku-data), CC BY-SA 4.0): 鎭西八郎爲朝外傳椿説弓張月（前編） p023 and p026, 公儀御触之部 p099, 千蟲譜 3巻 p077, 根無草後編 p016, 地震並出火細見記 p002 and 伊勢物語大全 p010. Lines where a transcriber used the mark for a single kana (た〲 for 但, こ〱ろ) were left out.

The settings were chosen by hand on a tuning page that draws the mark from the same outlines and transforms as the build, one judge, with GenZui Serif's mark available in the same passages for comparison. This is a direct setting by one reader, not a blind comparison. 〱: height 1,840, width 1.04, 20 units below centre, weight 53.4, upper arm turned −1.3°. 〲: the same stroke, dakuten at 1.11 of its scaled size, moved 35 units left and 15 up, turned 3°, weight 36. The two ballots and the outline hashes are in `votes/repeat-mark/`.
