...
def check(path: Path, family: str, full: bool):
    ...
    assert all(c in cmap for c in [*range(0x30A1, 0x30FB), *range(0x31F0, 0x3200), 0x3099, 0x309A, 0x309B, 0x309C, 0x30F0, 0x30F1, 0x30F2, 0x30F4, 0x1B127, 0x1B128, 0x2A708, 0x1B124, 0x1B125, 0x30FF, 0x1B122, 0x1B121, 0x1B126, 0x1B155, *range(0x1B164, 0x1B169), 0x30A0])
    if not full:
    ...
    for code in (*DIGRAPHS, 0x1B122, 0x1B121, 0x30A0):
        glyph = cmap[code]
